...
unsigned char *bmbuf[2] = {
    NULL, NULL}; /* two pbm bitmap lines with provision for leftskip */
...
void bitmap_seek(int offset) {
  ...
  if (offset) {
    while (offset > sizeof(garbage)) {
      size_t ret_code = fread(bmbuf[cbmbuf], 1, sizeof(garbage), bitmapf);
      offset -= sizeof(garbage);
    }
    size_t ret_code = fread(bmbuf[cbmbuf], 1, offset, bitmapf);
  }
}
...
void next_page(int page) {
  ...
  fprintf(stderr,
          "bmheight = %d, bmwidth = %d, leftskip = %d, topskip = %d, linecnt = "
          "%d, skip = %d\n",
...
unsigned char *get_line() {
  ...
  if (linecnt < (bmheight - topskip)) {
    if (bmwidth > 800) {
      size_t ret_code = fread(bmbuf[cbmbuf], 1, 800, bitmapf);
      bitmap_seek(bmwidth - 800);
    } else {
      size_t ret_code = fread(bmbuf[cbmbuf], 1, bmwidth, bitmapf);
...
int out_packet(int cnt, unsigned char c1, unsigned char c2, unsigned char c3,
               unsigned char c4) {
  if (cnt == 0) {
    if (c1 == 1) { /* band end */
      int startnext = 0;
...
int compress_bitmap() {
  ...
  if (strncmp((const char *) cbm[0], "P4", 2)) {
  ...
  do {
    char *tmp_chars;
    tmp_chars = fgets((char *) cbm[0], 200, bitmapf);
  } while (cbm[0][0] == '#');
  /* read the bitmap's dimensions */
  if (sscanf((const char *) cbm[0], "%d %d", &bmwidth, &bmheight) < 2) {
  ...
  *(cbmp++) = 0x0;
  ...
  /*printf("COMPRESS : bmcnt %d bmwidth %d bmheight %d csize %d linecnt %d
     pktcnt %d topskip %d leftskip %d\n", bmcnt, bmwidth, bmheight, csize,
     linecnt, pktcnt, topskip, leftskip);*/
  ...
  for (band = 0; linecnt < LINES_BY_PAGE; band++) {
    ...
    while (1) {
      ...
      // fprintf(stderr,"!cnt: %d, band: %d, linecnt: %d, diff :
...
void write_command_packet_buf(unsigned char one, unsigned char two, int uwait,
                              int nread, unsigned char *buf, int len) {
  int n, i, j;
...
int print_page(int page) {
  ...
  {
    unsigned char buf[] = {0x00, 0x00, 0xa4, 0x01, 0x02, 0x01, 0x00, 0x00, 0x1f,
                           0x1f, 0x1f, 0x1f, 0x00, 0x11, 0x03, 0x01, 0x01, 0x01,
                           0x02, 0x00, 0x00, 0x00, 0x70, 0x00, 0x78, 0x00, 0x50,
                           0x02, 0x7a, 0x1a, 0x60, 0x13, 0x67, 0x1b};
  ...
  for (ccbm = 0; cbm[ccbm] != NULL; ccbm++) {
    ...
    /*printf("First ten bytes: %x %x %x %x %x %x %x %x %x %x\n",
            cbm[ccbm][0], cbm[ccbm][1], cbm[ccbm][2], cbm[ccbm][3],
       cbm[ccbm][4], cbm[ccbm][5], cbm[ccbm][6], cbm[ccbm][7], cbm[ccbm][8],
       cbm[ccbm][9]);*/
  ...
  while (1) {
    ...
    /*			strcpy(gname,"/tmp/lbp810-XXXXXX");
                            if ((tfd=mkstemp(gname))<0) {
                                    fprintf(stderr,"Can't open a temporary
       file.\n"); errorexit();
                            }
                            cbmf = fdopen(tfd,"w+");*/
    if (!compress_bitmap())
